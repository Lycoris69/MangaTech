...
exports.HotScansExtractor = void 0;
const cheerio = __importStar(require("cheerio"));
const axios_1 = __importDefault(require("axios"));
const Logger_1 = require("./Logger");
const interfaces_1 = require("./interfaces");
class HotScansExtractor {
    constructor(urlManager, rateLimiter, contentValidator) {
        this.cache = new Map();
        this.CACHE_TTL = 30 * 60 * 1000; // 30 minutes cache TTL
        this.urlManager = urlManager;
        this.rateLimiter = rateLimiter;
        this.contentValidator = contentValidator;
        // Initialize logger
        if (!HotScansExtractor.logger) {
            HotScansExtractor.logger = Logger_1.Logger.create('hot-scans-extractor');
        }
    ...
    async extractHotScans() {
        ...
        try {
            ...
            if (!validationResult.isValid) {
                throw new interfaces_1.ValidationError(`Hot scans validation failed: ${validationResult.errors.join(', ')}`);
            }
            // Cache the results
            ...
            throw new interfaces_1.ScrapingError(`Failed to extract hot scans: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     * Parse the hot scans/trending section from the homepage HTML
     */
    async parseHotScansSection($) {
        const hotScans = [];
     ...
     */
    parseHotScanItem($, element, index) {
        try {
...
exports.HotScansExtractor = HotScansExtractor;
//# sourceMappingURL=HotScansExtractor.js.map