...
export class URLManager {
  ...
  buildChapterUrl(chapterId: string): string {
    ...
    // ManhwaZ uses /webtoon/SERIES-SLUG/CHAPTER-SLUG for chapter pages
    if (cleanId.includes('/')) {
      return `${this.baseUrl}/webtoon/${cleanId}`
    }

    // If no slash, we assume it's just the chapter slug (legacy or fallback)
  ...
  extractChapterId(url: string): string | null {
    ...
    try {
      const parsedUrl = new URL(url)
      // Check for /webtoon/SERIES/CHAPTER pattern
      const webtoonMatch = parsedUrl.pathname.match(/\/webtoon\/([^/]+\/[^/]+)/)
      if (webtoonMatch) return webtoonMatch[1]

      // Fallback for legacy /chapter/ pattern
      const chapterMatch = parsedUrl.pathname.match(/\/chapter\/([^/]+)/)
