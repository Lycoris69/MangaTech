...
import { DownloadTask, PageUrl } from '../types/index.js'
...
export class DownloadManager {
    ...
    private onTaskUpdate: ((tasks: DownloadTask[]) => void) | null = null
    ...
    public setOnTaskUpdate(callback: (tasks: DownloadTask[]) => void): void {
        ...
        try {
            ...
            if (task && task.status === 'completed' && fs.existsSync(task.downloadPath)) {
                ...
                for (const file of files) {
                    const filePath = path.join(task.downloadPath, file)
                    const index = files.indexOf(file)
                    
                    try {
                        const metadata = await sharp(filePath).metadata()
                        const relativePath = path.relative(path.dirname(path.dirname(task.downloadPath)), task.downloadPath);
                        pages.push({
                            pageNumber: index + 1,
                            imageUrl: `/static/manga/${encodeURIComponent(path.basename(path.dirname(task.downloadPath)))}/${encodeURIComponent(path.basename(task.downloadPath))}/${encodeURIComponent(file)}`,
                            localPath: filePath,
                            width: metadata.width,
                            height: metadata.height
                        })
                    } catch (err) {
                        DownloadManager.logger.error(`Failed to get dimensions for local file: ${filePath}`, err)
                        pages.push({
                            pageNumber: index + 1,
                            imageUrl: `/static/manga/${encodeURIComponent(path.basename(path.dirname(task.downloadPath)))}/${encodeURIComponent(path.basename(task.downloadPath))}/${encodeURIComponent(file)}`,
                            localPath: filePath
